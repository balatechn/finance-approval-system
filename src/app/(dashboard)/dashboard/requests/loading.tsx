  ...
  return (
    <div className="space-y-6 animate-pulse">
      ...
      <div className="flex gap-4">
        <div className="h-10 w-48 rounded bg-white/40" />
        <div className="h-10 w-40 rounded bg-white/40" />
        <div className="ml-auto h-10 w-20 rounded bg-white/40" />
      </div>
          ...
          <div className="space-y-4">
            <div className="flex gap-4 border-b pb-3">
              {["w-24", "flex-1", "w-24", "w-20", "w-20", "w-16", "w-12"].map((w, i) => (
                <div key={i} className={`h-4 ${w} rounded bg-white/40`} />
              ))}
            </div>
            {[...Array(6)].map((_, i) => (
              <div key={i} className="flex gap-4 py-1">
                {["w-24", "flex-1", "w-24", "w-20", "w-20", "w-16", "w-12"].map((w, j) => (
                  <div key={j} className={`h-4 ${w} rounded bg-white/40`} />
                ))}
              </div>
            ))}
          </div>
