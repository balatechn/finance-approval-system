          ...
          <CardContent className="space-y-4">
            ...
            >
              ...
              <p className="text-xs text-muted-foreground mt-1">
                PDF, Images, Word, Excel — up to 5MB per file
              </p>
              <input
                ref={fileInputRef}
                type="file"
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  {filePreviews.map((item, index) => (
                    ...
                    >
                      ...
                      <div className="flex-shrink-0 h-16 w-16 rounded-md overflow-hidden bg-white/70 backdrop-blur-sm flex items-center justify-center">
                        {item.type.startsWith('image/') && item.preview ? (
                          <img
                            ...
                            onClick={(e) => {
                              e.stopPropagation()
                              setPreviewFile({ url: item.preview, type: item.type, name: item.file.name })
                            }}
                          />
                      ...
                      <button
                        type="button"
                        className="absolute top-2 right-2 rounded-full p-1 text-gray-400 hover:text-red-600 hover:bg-red-100 transition-colors"
                        onClick={(e) => {
                          e.stopPropagation()
                          removeFile(index)
                        }}
